...
public class ReversingBinaryNumber implements Runnable {
	private static int reverseBinary(int n) {
		long rev = 0;
		int size = (int)(Math.log10(n)/Math.log(2));
		while (n > 0) {
			rev <<= 1;
			rev |= n & 1;
			n >>= 1;
    ...
    private static final void solution() throws IOException {
        ...
        while (test-- > 0) {
            ...
            output.println("Init. Binary: " + Integer.toBinaryString(num));
            num = reverseBinary(num);
            output.println("Reverse Binary: " + Integer.toBinaryString(num));
        }
        output.close();
        input.close();
    }
    ...
    public static void main(String... args) throws IOException {
        new Thread(null, new ReversingBinaryNumber(), "Main", 1 << 26).start();
    }
