...
public class OperationsOnKthBit implements Runnable {
    ...
    private static final void solution() throws IOException {
        ...
        while (test-- > 0) {
            ...
            k = input.nextInt();
            output.println("\nN = " + number + String.format(" {%s}, K = %d", 
            			Integer.toBinaryString(number), k));
            output.println("Is Kth Bit set or not? " + isKthBitSet(number, k));
            output.println("After Setting Kth Bit: " + 
            			Integer.toBinaryString(setKthBit(number, k)));
            output.println("After Clearing Kth Bit: " + 
            			Integer.toBinaryString(clearKthBit(number, k)));
            output.println("After Toggling Kth Bit: " + 
            			Integer.toBinaryString(toggleKthBit(number, k)));
        }
        output.close();
        input.close();
    }
    ...
    public static void main(String... args) throws IOException {
        new Thread(null, new OperationsOnKthBit(), "Main", 1 << 26).start();
    }
