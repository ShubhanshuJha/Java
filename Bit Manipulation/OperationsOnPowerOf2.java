...
public class OperationsOnPowerOf2 implements Runnable {
	private static boolean isPowerOfTwo(int n) {
		return (n & --n) == 0;
	}
	private static int multiplyByPowOf2(int n, int k) {
		return n << k; // return n * (2^k)
	}
	private static int divideByPowOf2(int n, int k) {
		return n >> k; // return n / (2^k)
	}
    private static final void solution() throws IOException {
        ...
        while (test-- > 0) {
            int n = input.nextInt();
            int k = input.nextInt();
            output.println("Is N a Power Of Two? " + isPowerOfTwo(n));
            output.println("After multiplying number by Power Of 2: " +
            		multiplyByPowOf2(n, k));
            output.println("After dividing number by Power Of 2: " +
            		divideByPowOf2(n, k));
        }
        output.close();
        input.close();
    ...
    public static void main(String... args) throws IOException {
        new Thread(null, new OperationsOnPowerOf2(), "Main", 1 << 26).start();
    }
