...
public class ChangeBits implements Runnable {
	private static int changeBit(int n) {
		int count = 0;
		while (n > 0) {
			count++;
			n >>= 1;
    ...
    private static final void solution() throws IOException {
        ...
        while (test-- > 0) {
            int n = input.nextInt(); //n=6 -> 7 and n=8 -> 15
            output.println("N = " + n + " {" + Integer.toBinaryString(n)
            		+ "}");
            var result = changeBit(n);
            output.println("After changing 0 to 1: " +
                    result + " {" + 
    ...
    public static void main(String... args) throws IOException {
        new Thread(null, new ChangeBits(), "Main", 1 << 26).start();
    }
