...
public class MaskTrailingZeros implements Runnable {
	private static int maskTrailingZeros(int n) {
		return (n & -n) - 1;
	}
    private static final void solution() throws IOException {
        ...
        while (test-- > 0) {
            int n = input.nextInt();
            output.println("N = " + n + " {" + Integer.toBinaryString(n)
            		+ "}");
            output.println("After masking trailing zeros: " +
            		Integer.toBinaryString(maskTrailingZeros(n)));
        }
        output.close();
        input.close();
    }
    ...
    public static void main(String... args) throws IOException {
        new Thread(null, new MaskTrailingZeros(), "Main", 1 << 26).start();
    }
