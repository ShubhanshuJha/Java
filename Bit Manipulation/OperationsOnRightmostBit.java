...
public class OperationsOnRightmostBit implements Runnable {
	private static int toggleRightMostOneBit(int n) {
		return n & --n;
	}
	private static int isolateRightMostOneBit(int n) {
		return n & ((~n) + 1);
	}
	private static int isolateRightMostZeroBit(int n) {
		return ~n & (n + 1);
	}
    private static final void solution() throws IOException {
        ...
        while (test-- > 0) {
            ...
            output.println("N = " + num + " {" + 
            	Integer.toBinaryString(num) + "}");
            output.println("After Toggling Rightmost One Bit: " +
            		Integer.toBinaryString(toggleRightMostOneBit(num)));
            output.println("After Isolating Rightmost One Bit: " +
            		Integer.toBinaryString(isolateRightMostOneBit(num)));
            output.println("After Isolating Rightmost Zero Bit: " +
            		Integer.toBinaryString(isolateRightMostZeroBit(num)));
        }
        output.close();
        input.close();
    }
    ...
    public static void main(String... args) throws IOException {
        new Thread(null, new OperationsOnRightmostBit(), "Main", 1 << 26).start();
    }
