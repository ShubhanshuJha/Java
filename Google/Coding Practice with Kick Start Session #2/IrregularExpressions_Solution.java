...
public class IrregularExpressions_Solution implements Runnable {

    private static final String[] result = {"Spell!", "Nothing."};
    ...
    private static final void solution() throws IOException {
        ...
        for (int t = 1; t <= test; t++) {
            ...
            if (checkSpell(str, n))
                resIdx = 0;
            output.println("Case #" + t + ": " + result[resIdx]);
        }
        output.close();
        input.close();
    }
    private static final boolean checkSpell(String spell, int n) {
        List<Integer> vowIdx = new ArrayList<>();
        for (int i = 0; i < n; i++) {
            if (isVowel(spell.charAt(i)))
                vowIdx.add(i);
        }
        if (vowIdx.size() <= 4) return false;

        for (int i = 0; i < vowIdx.size() - 4; i++) {
            int fV = vowIdx.get(i),
                sV = vowIdx.get(i + 1);
            String startWord = spell.substring(fV, sV + 1);

            int thirdVowel = vowIdx.get(i + 2);
            // String restSpell = spell.substring(thirdVowel + 1, n);
            // if (restSpell.contains(startWord)) return true;
            if (spell.indexOf(startWord, thirdVowel + 1) != -1) return true;
        }
    ...
    public static void main(String... args) throws IOException {
        new Thread(null, new IrregularExpressions_Solution(), "Main", 1 << 26).start();
    }
