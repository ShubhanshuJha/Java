...
public class SampleProblem_Solution implements Runnable {
    private static final void solution() throws IOException {
        ...
        for (int t = 1; t <= test; t++) {
            int N = input.nextInt(),
            M = input.nextInt();
            int[] candies = input.readIntArray(N);
            int result = Arrays.stream(candies).sum() % M;
    ...
    public static void main(String... args) throws IOException {
        new Thread(null, new SampleProblem_Solution(), "Main", 1 << 26).start();
    }
